...
class Video(Thread):
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            ...
            publish.single("mydata/stream/camera/BTX5860WE168", byteData, hostname="15.165.185.201") # Broker로 전송 (CLOUD)
            if accumulated_time > 10:
                print(f'accumulated time : {accumulated_time}, send img to AI model')
                accumulated_time = 0
                publish.single("mydata/fire/BTX5860WE168", byteData, hostname="15.165.185.201") # Broker로 전송 (AI)
